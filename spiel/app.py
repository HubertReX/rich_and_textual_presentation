...
class SpielApp(App[None]):
    CSS_PATH = "spiel.css"
    BINDINGS = [
        Binding("q,esc", "quit", "Quit slideshow."),
    ...
    async def action_first_slide(self) -> None:
        await self.handle_new_slide(0, Direction.Previous)

    async def action_last_slide(self) -> None:
        await self.handle_new_slide(len(self.deck) - 1, Direction.Next)
...
def present(deck_path: Path | str, watch_path: Path | str | None = None) -> SpielApp:
    """
    ...
    return SpielApp(deck_path=deck_path, watch_path=watch_path).run()

def init_app(deck_path: Path | str, watch_path: Path | str | None = None) -> SpielApp:
    """
    Present the deck defined in the given `deck_path`.
    ...
    return SpielApp(deck_path=deck_path, watch_path=watch_path)
